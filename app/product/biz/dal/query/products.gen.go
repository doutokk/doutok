...
func newProduct(db *gorm.DB, opts ...gen.DOOption) product {
	...
	_product.Categories = productManyToManyCategories{
...
type product struct {
	...
	Price       field.Float32
	Categories  productManyToManyCategories
...
type productManyToManyCategories struct {
	db *gorm.DB
...
func (a productManyToManyCategories) WithContext(ctx context.Context) *productManyToManyCategories {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a productManyToManyCategories) Session(session *gorm.Session) *productManyToManyCategories {
	a.db = a.db.Session(session)
	return &a
}

func (a productManyToManyCategories) Model(m *model.Product) *productManyToManyCategoriesTx {
	return &productManyToManyCategoriesTx{a.db.Model(m).Association(a.Name())}
}

type productManyToManyCategoriesTx struct{ tx *gorm.Association }

func (a productManyToManyCategoriesTx) Find() (result []*model.ProductCategory, err error) {
	return result, a.tx.Find(&result)
}

func (a productManyToManyCategoriesTx) Append(values ...*model.ProductCategory) (err error) {
...
func (a productManyToManyCategoriesTx) Replace(values ...*model.ProductCategory) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a productManyToManyCategoriesTx) Delete(values ...*model.ProductCategory) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a productManyToManyCategoriesTx) Clear() error {
	return a.tx.Clear()
}

func (a productManyToManyCategoriesTx) Count() int64 {
	return a.tx.Count()
}
...
func (p productDo) GetByUserId(userId uint32) (result []*model.Product, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, userId)
	generateSQL.WriteString("SELECT * FROM products WHERE user_id = ? and deleted_at is null ")
